# ==========================================================================
# This program is developed to find theoretical CT for SJ.
...
# font_import() 
  ...
  fileName1 <- paste(getwd(), "/SJ01.DataSeg", saveDate, ".xlsx", sep="")
  lWB <- loadWorkbook(fileName1, create = TRUE)
  for(index in 1:length(nameData)) {
    sourceData[[index]] <- subset(x=rawData, subset=(rawData$"Product"==nameData[index]), select=c(Product, Type, Date, Part, Machine, Throughput))
  ...
  fileName2 <- paste(getwd(), "/SJ02.Boxplot", saveDate, ".pdf", sep="")
  ...
  for(index in 1:length(nameData)) {
    par(mfrow=c(2,2))
    plot(as.Date(sourceData[[index]]$"Date"), jitter(sourceData[[index]]$"Throughput"), main="", xlab="Date", ylab="Throughput", xaxt="n")
    title(main=paste("Trend by date of ", nameData[index]), family="Korea1deb", font.main=1, col.main="blue")
    ...
    boxplot(Throughput ~ Type, data=sourceData[[index]], main="Type vs. Throughput", xlab="Type", ylab="Throughput")
    boxplot(Throughput ~ Machine, data=sourceData[[index]], main="Machine vs. Throughput", xlab="Machine", ylab="Throughput")
    boxplot(Throughput ~ Part, data=sourceData[[index]], main="Part vs. Throughput", xlab="Part", ylab="Throughput")
    par(new=F)
  }
  dev.off()
  
  fileName3    <- paste(getwd(), "/SJ03.ResultCT", saveDate, ".xlsx", sep="")
